...
from kivy.properties import ListProperty, NumericProperty, StringProperty, ColorProperty, BooleanProperty
...
class Toast(ModalView):
    ...
    def __init__(self, **kwargs):
        ...
        self.label_toast = Label(size_hint=(None, None), opacity=0, color=self.color, bold=self.bold)
...
@mainthread
def toast(text: str = "", background: list = None, duration: float = 5, text_color=None, bold=False) -> None:
    """
    Displays a toast.
    ...
    if background is None:
        background = [0, 0, 0, .7]
    Toast(duration=duration, _md_bg_color=background, color=text_color, bold=bold).toast(text)
